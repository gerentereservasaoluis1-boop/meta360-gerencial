...
export default function App(){
  ...
  var mediaAnual=MS.map(function(_m,i){
    ...
    var mediaMedia=vendeuM!=null?(vendeuM/diasP):null; /* media real por dia trabalhado */
    var mediaMesM=entry.mediaMes!=null?entry.mediaMes:null; /* saved monthly avg */
    return {mes:i,metaM:metaM,vendeuM:vendeuM,resultado:resultado,pctM:pctM,mediaMedia:mediaMedia,mediaMesM:mediaMesM,key:key,isAuto:i===mesG&&totalVendasG>0};
  });
              ...
              return (
                <div key={i} style={{borderBottom:i<11?"1px solid "+G.border+"55":"none",background:isAtual?"rgba(34,197,94,.05)":"transparent"}}>
                  <div style={{display:"grid",gridTemplateColumns:"38px 1fr 1fr 48px 48px",gap:3,padding:"8px 10px",alignItems:"center"}}>
                    ...
                    {/* % MES - percentual do mes, auto-calculado e salvo */}
                    {(function(){
                      /* pct do mes = vendido/meta*100, auto quando tem vendido+meta */
                      var autoPct=m.vendeuM!=null&&(m.metaM!=null?m.metaM:metaLoja)>0
                        ?(m.vendeuM/(m.metaM!=null?m.metaM:metaLoja)*100):null;
                      var pctDisplay=autoPct;
                      var pc2=pctDisplay!=null?(pctDisplay>=100?"#4ade80":pctDisplay>=85?G.amber:G.red):G.muted2;
                      return <div style={{textAlign:"center"}}>
                        {pctDisplay!=null
                          ? <div style={{fontSize:11,fontWeight:900,color:pc2,lineHeight:1}}>
