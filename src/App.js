...
export default function App(){
  ...
  /* vendas por dia do gerente */
  var vendasGSt=useState(function(){return loadLS("m360ger_vendas",{});});
  var vendasG=vendasGSt[0]; var setVendasG=vendasGSt[1];
  function setVendasGFn(fn){ setVendasG(function(p){ var n=typeof fn==="function"?fn(p):fn; saveLS("m360ger_vendas",n); return n; }); }
  var hoje2=new Date();
  var anoG=hoje2.getFullYear(); var mesG=hoje2.getMonth();
  ...
  var showAtuSt=useState(false); var showAtu=showAtuSt[0]; var setShowAtu=showAtuSt[1];
  var atuDiaSt=useState(""); var atuDia=atuDiaSt[0]; var setAtuDia=atuDiaSt[1];
  var atuQtdSt=useState(""); var atuQtd=atuQtdSt[0]; var setAtuQtd=atuQtdSt[1];
    ...
    <div style={{minHeight:"100vh",background:G.bg,color:G.white,fontFamily:"'Sora',sans-serif",paddingBottom:90,maxWidth:420,margin:"0 auto",position:"relative",overflow:"hidden"}}>
      ...
      {page==="medias"&&(
        <div className="fu" style={{position:"relative",zIndex:1,padding:"52px 20px 0"}}>
          ...
          {/* cabeçalho tabela */}
          <div style={{display:"grid",gridTemplateColumns:"40px 1fr 1fr 52px 52px",gap:3,padding:"7px 10px",background:G.card2,borderRadius:"10px 10px 0 0",border:"1px solid "+G.border,borderBottom:"none"}}>
            {["MES","META","VENDIDO","MEDIA","RESULT."].map(function(h,hi){ return <div key={hi} style={{fontSize:7,color:G.muted,fontWeight:700,textAlign:hi>0?"center":"left"}}>{h}</div>; })}
          </div>
          {/* linhas */}
          <div style={{background:G.card,border:"1px solid "+G.border,borderRadius:"0 0 18px 18px",overflow:"hidden",marginBottom:10}}>
            {mediaAnualG.map(function(m,i){
              ...
              /* auto-calculate vendido from vendasG for current month */
              var vendeuAuto=isAtual&&totalVendasG>0?totalVendasG:m.vendeuM;
              var metaAuto=m.metaM!=null?m.metaM:metaLoja;
              var pctAuto=metaAuto>0&&vendeuAuto!=null?(vendeuAuto/metaAuto)*100:m.pctM;
              var resultAuto=vendeuAuto!=null?vendeuAuto-metaAuto:m.resultado;
              var pc=pctAuto!=null?(pctAuto>=100?G.green:pctAuto>=65?"#f59e0b":"#ef4444"):G.muted2;
              var rc=resultAuto!=null?(resultAuto>=0?G.green:"#ef4444"):G.muted2;
              ...
              /* media diaria = vendido / dias trabalhados ate hoje */
              var diasPassados=isAtual?Math.max(1,hoje.getDate()):new Date(anoAtualG,i+1,0).getDate();
              var mediaDia=vendeuAuto!=null&&diasPassados>0?vendeuAuto/diasPassados:null;
              return (
                <div key={i} style={{borderBottom:i<11?"1px solid "+G.border+"66":"none",background:isAtual?"rgba(34,197,94,.06)":"transparent"}}>
                  <div style={{display:"grid",gridTemplateColumns:"40px 1fr 1fr 52px 52px",gap:3,padding:"9px 10px",alignItems:"center"}}>
                    ...
                    <div style={{textAlign:"center"}}>
                      {eM
                        ...
                        : <button onClick={function(){setMediaEditG({key:m.key,field:"meta"});setMediaValG(m.metaM!=null?String(m.metaM):"");}}
                            style={{background:"transparent",border:"1px solid "+(m.metaM!=null?"rgba(255,255,255,.15)":G.border),borderRadius:6,padding:"4px 5px",color:m.metaM!=null?"rgba(255,255,255,.6)":G.muted2,fontSize:9,fontWeight:700,width:"100%"}}>
                            {m.metaM!=null?fRk(m.metaM):"—"}
                          </button>
                      }
                    </div>
                    {/* vendido - auto para mes atual */}
                    <div style={{textAlign:"center"}}>
                      {isAtual&&totalVendasG>0
                        ? <div style={{background:pc+"18",border:"1px solid "+pc+"44",borderRadius:6,padding:"4px 5px",color:pc,fontSize:9,fontWeight:900}}>
                            {fRk(totalVendasG)}
                            <div style={{fontSize:6,color:pc+"99",marginTop:1}}>auto</div>
                          </div>
                        : eV
                          ? <input autoFocus type="text" inputMode="decimal" value={mediaValG}
                              onChange={function(e){setMediaValG(e.target.value);}}
                          ...
                          : <button onClick={function(){setMediaEditG({key:m.key,field:"vendeu"});setMediaValG(m.vendeuM!=null?String(m.vendeuM):"");}}
                              style={{background:m.vendeuM!=null?pc+"18":"transparent",border:"1px solid "+(m.vendeuM!=null?pc+"44":G.border),borderRadius:6,padding:"4px 5px",color:m.vendeuM!=null?pc:G.muted2,fontSize:9,fontWeight:700,width:"100%"}}>
                    ...
                    <div style={{textAlign:"center"}}>
                      {mediaDia!=null
                        ? <div style={{fontSize:9,fontWeight:900,color:"rgba(255,255,255,.6)",lineHeight:1}}>{fRk(mediaDia)}<div style={{fontSize:6,color:G.muted,marginTop:1}}>/dia</div></div>
                    ...
                    <div style={{textAlign:"center"}}>
                      {resultAuto!=null
                        ? <div style={{fontSize:9,fontWeight:900,color:rc,lineHeight:1}}>{resultAuto>=0?"+":""}{fRk(resultAuto)}</div>
                        : <div style={{fontSize:9,color:G.muted2}}>—</div>
                      }
                    </div>
                  </div>
                  {pctAuto!=null&&(
                    <div style={{padding:"0 10px 7px"}}>
                      <div style={{height:3,background:G.muted2,borderRadius:2,overflow:"hidden"}}>
                        <div style={{height:"100%",width:Math.min(100,pctAuto)+"%",background:pc,borderRadius:2}}/>
                      </div>
                    </div>
                  )}
                </div>
      ...
      {page==="config"&&(
        <div className="fu" style={{position:"relative",zIndex:1,padding:"52px 20px 0"}}>
          ...
          {/* ATALHO ATUALIZAR */}
          <div onClick={function(){setShowAtu(true);setAtuDia("");setAtuQtd("");setPage("home");}} style={{background:"linear-gradient(135deg,#0a1a0a,#0f1f0f)",border:"1px solid rgba(34,197,94,.3)",borderRadius:16,padding:"14px 16px",marginBottom:12,display:"flex",alignItems:"center",gap:14,cursor:"pointer"}}>
            <div style={{width:40,height:40,borderRadius:12,background:"rgba(34,197,94,.15)",display:"flex",alignItems:"center",justifyContent:"center",fontSize:20}}>🔄</div>
            <div style={{flex:1}}>
      ...
      {/* ══ MODAL ATUALIZAR VENDAS POR DIA ══ */}
      {showAtu&&(
        <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,.9)",display:"flex",alignItems:"flex-end",justifyContent:"center",zIndex:250,backdropFilter:"blur(12px)"}} onClick={function(){setShowAtu(false);setAtuDia("");setAtuQtd("");}}>
          <div className="fu" style={{background:G.card,borderRadius:"24px 24px 0 0",padding:"16px 20px 44px",width:"100%",maxWidth:420,border:"1px solid rgba(255,255,255,.1)",borderBottom:"none"}} onClick={function(e){e.stopPropagation();}}>
            ...
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:14}}>
              <div>
                ...
                <div style={{fontSize:20,fontWeight:900}}>{atuDia?"Dia "+parseInt(atuDia.split("-")[2])+" - Lancar":"Selecione um dia"}</div>
              </div>
              {atuDia&&vendasG[atuDia]>0&&<div style={{background:"rgba(255,255,255,.1)",borderRadius:8,padding:"4px 10px",fontSize:10,color:G.white,fontWeight:700}}>
                Atual: {fR(vendasG[atuDia]||0)}
              </div>}
            </div>
            {/* seletor de dias */}
            <div style={{marginBottom:14}}>
              <div style={{fontSize:9,color:G.muted,fontWeight:600,marginBottom:6}}>DIA DO MES</div>
              <div style={{display:"flex",gap:5,overflowX:"auto",paddingBottom:4}}>
                {(function(){
                  var days2=[];
                  for(var di=1;di<=diasNoMesG;di++){
                    var ds3=prefixG+"-"+String(di).padStart(2,"0");
                    var sel=atuDia===ds3;
                    var isToday2=di===hoje2.getDate();
                    var temVenda=vendasG[ds3]>0;
                    var semVenda=!temVenda&&di<hoje2.getDate();
                    days2.push(<button key={di} onClick={function(k3){return function(){setAtuDia(k3);setAtuQtd(vendasG[k3]>0?String(vendasG[k3]):"");};}(ds3)}
                      style={{flexShrink:0,width:44,height:52,borderRadius:10,
                        border:"2px solid "+(sel?"#fff":isToday2?"rgba(255,255,255,.5)":temVenda?"rgba(255,255,255,.3)":semVenda?"rgba(239,68,68,.4)":"rgba(255,255,255,.1)"),
                        background:sel?"rgba(255,255,255,.2)":isToday2?"rgba(255,255,255,.08)":temVenda?"rgba(255,255,255,.06)":semVenda?"rgba(239,68,68,.08)":"transparent",
                        color:sel?"#fff":isToday2?"rgba(255,255,255,.9)":temVenda?"rgba(255,255,255,.7)":semVenda?"rgba(239,68,68,.8)":"rgba(255,255,255,.3)",
                        fontSize:12,fontWeight:700,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",gap:1}}>
                      <span style={{lineHeight:1}}>{di}</span>
                      {temVenda&&<span style={{fontSize:6,color:sel?"#fff":"rgba(255,255,255,.5)"}}>{fRk(vendasG[ds3]).replace("R$","")}</span>}
                      {!temVenda&&semVenda&&<span style={{fontSize:6,color:"rgba(239,68,68,.8)",fontWeight:800}}>!</span>}
                      {isToday2&&!sel&&<span style={{fontSize:5,color:"rgba(255,255,255,.4)"}}>hoje</span>}
                    </button>);
                  }
                  return days2;
                })()}
            ...
            {atuDia&&(
              <div>
                <div style={{fontSize:9,color:G.muted,fontWeight:600,marginBottom:6}}>VALOR EM R$ — DIA {parseInt(atuDia.split("-")[2])}</div>
                <div style={{position:"relative",marginBottom:8}}>
                ...
                {(function(){
                  var v2=pRS(atuQtd); if(!v2||v2<=0) return null;
                  var totalNovo=totalVendasG - (vendasG[atuDia]||0) + v2;
                  var p2=metaLoja>0?(totalNovo/metaLoja)*100:0;
                  return <div style={{textAlign:"center",marginBottom:10,fontSize:11,fontWeight:700,color:p2>=100?G.green:G.amber}}>
                    {p2>=100?"✅ Meta batida! Total: "+fR(totalNovo):"📌 Total do mes: "+fR(totalNovo)+" ("+p2.toFixed(1)+"%)"}
                  </div>;
                })()}
                <div style={{display:"flex",gap:8}}>
                  ...
                  <button className="tap" onClick={function(){
                    var v4=pRS(atuQtd);
                    setVendasGFn(function(p){ var n=Object.assign({},p); if(atuQtd===""){ delete n[atuDia]; }else{ n[atuDia]=v4; } return n; });
                    /* atualiza total no config */
                    setTimeout(function(){
                      var allV=loadLS("m360ger_vendas",{});
                      var total=Object.keys(allV).filter(function(k){return k.indexOf(prefixG)===0;}).reduce(function(a,k){return a+(allV[k]||0);},0);
                      setCfg({vendidoStr:String(total)});
                    },100);
            ...
            {!atuDia&&<div style={{textAlign:"center",padding:"14px 0",color:G.muted,fontSize:12}}>
              <div style={{fontSize:24,marginBottom:6}}>👆</div>Toque em um dia para lancar o faturamento
            </div>}
          </div>
        </div>
      )}
