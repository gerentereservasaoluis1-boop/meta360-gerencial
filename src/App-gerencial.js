...
export default function App(){
  ...
  return (
    <div style={{minHeight:"100vh",background:G.bg,color:G.white,fontFamily:"'Sora',sans-serif",paddingBottom:90,maxWidth:420,margin:"0 auto",position:"relative",overflow:"hidden"}}>
      ...
      {/* ══ MODAL ATUALIZAR ══ */}
      {showAtu&&(
        <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,.88)",display:"flex",alignItems:"flex-end",justifyContent:"center",zIndex:250,backdropFilter:"blur(10px)"}} onClick={function(){setShowAtu(false);setAtuDia("");setAtuQtd("");}}>
          <div className="fu" style={{background:G.card,borderRadius:"24px 24px 0 0",padding:"16px 20px 44px",width:"100%",maxWidth:420,border:"1px solid rgba(255,255,255,.1)",borderBottom:"none"}} onClick={function(e){e.stopPropagation();}}>
            ...
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:14}}>
              <div>
                <div style={{fontSize:9,color:G.muted,fontWeight:600,letterSpacing:1}}>LANCAR FATURAMENTO</div>
                <div style={{fontSize:20,fontWeight:900}}>{atuDia?"Dia "+parseInt(atuDia.split("-")[2]):"Selecione um dia"}</div>
              </div>
            </div>
            {/* dias do mes */}
            <div style={{marginBottom:14}}>
              ...
              <div style={{display:"flex",gap:5,overflowX:"auto",paddingBottom:4}}>
                {(function(){
                  var hoje2=new Date();
                  var diasNoMes2=new Date(hoje2.getFullYear(),hoje2.getMonth()+1,0).getDate();
                  var days=[];
                  for(var di=1;di<=diasNoMes2;di++){
                    var ds2=hoje2.getFullYear()+"-"+String(hoje2.getMonth()+1).padStart(2,"0")+"-"+String(di).padStart(2,"0");
                    var sel=atuDia===ds2;
                    var isToday=di===hoje2.getDate();
                    days.push(<button key={di} onClick={function(d2,k2){return function(){setAtuDia(k2);setAtuQtd("");};}(di,ds2)}
                      style={{flexShrink:0,width:42,height:48,borderRadius:10,
                        border:"2px solid "+(sel?"rgba(255,255,255,.8)":isToday?"rgba(255,255,255,.4)":"rgba(255,255,255,.1)"),
                        background:sel?"rgba(255,255,255,.2)":isToday?"rgba(255,255,255,.08)":"transparent",
                        color:sel?"#fff":isToday?"rgba(255,255,255,.8)":"rgba(255,255,255,.4)",
                        fontSize:13,fontWeight:700,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",gap:2}}>
                      <span>{di}</span>
                      {isToday&&!sel&&<span style={{fontSize:6,color:"rgba(255,255,255,.5)"}}>hoje</span>}
                    </button>);
                  }
                  return days;
                })()}
            ...
            {atuDia&&(
              <div>
                <div style={{fontSize:9,color:G.muted,fontWeight:600,marginBottom:6}}>VALOR EM R$</div>
                <div style={{position:"relative",marginBottom:8}}>
                  ...
                  <input type="text" inputMode="decimal" value={atuQtd} autoFocus
                    ...
                    onKeyDown={function(e){if(e.key==="Enter"){var v=pRS(atuQtd);if(v>=0){setCfg({vendidoStr:String(v)});setShowAtu(false);setAtuDia("");setAtuQtd("");};}}}
                    placeholder="0,00"
                    style={{width:"100%",background:G.card2,border:"2px solid rgba(255,255,255,.2)",borderRadius:14,padding:"16px 14px 16px 50px",fontSize:36,fontWeight:900,color:G.white,letterSpacing:-1}}/>
                </div>
                {(function(){
                  var v2=pRS(atuQtd); if(!v2||v2<=0) return null;
                  var p2=metaLoja>0?(v2/metaLoja)*100:0;
                  return <div style={{textAlign:"center",marginBottom:10,fontSize:11,fontWeight:700,color:p2>=100?G.green:G.amber}}>
                    {p2>=100?"✅ Meta batida!":"📌 "+p2.toFixed(1)+"% da meta"}
                  </div>;
                })()}
                <div style={{display:"flex",gap:8}}>
                  ...
                  <button className="tap" onClick={function(){
                    var v3=pRS(atuQtd);
                    if(v3>=0){setCfg({vendidoStr:String(v3)});}
            ...
            {!atuDia&&<div style={{textAlign:"center",padding:"16px 0",color:G.muted,fontSize:12}}>
              <div style={{fontSize:24,marginBottom:8}}>👆</div>Toque em um dia para lancar
            </div>}
          </div>
        </div>
      )}
